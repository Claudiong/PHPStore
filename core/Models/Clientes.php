...
class Clientes
{
   ...
   public function verificar_email_existe_base($email, $id_cliente)
   {
      ...
      $resultados = $bd->select(
         "SELECT ID_CLIENTE, EMAIL FROM CLIENTES WHERE EMAIL = :email and ID_CLIENTE<>:id_cliente",
         $parametros
      );
   ...
   public function validar_email($purl)
   {
      ...
      $resultados = $bd->select(
         "SELECT id_cliente FROM CLIENTES WHERE purl = :purl",
         $parametros
      );
   ...
   public function validar_login($usuario, $senha)
   {
      ...
      $resultados = $bd->select(
         "SELECT * FROM CLIENTES WHERE EMAIL = :usuario and ativo=1 and deleted_at is null",
         $parametros
      );
      if (count($resultados) != 1) {
         return false;
   ...
   public function atualizar_nova_senha($id_cliente, $nova_senha)
   {
      ...
      // $bd = new Database();
      $this->bd2->update('UPDATE CLIENTES SET SENHA=:senha, updated_at = NOW() WHERE 
